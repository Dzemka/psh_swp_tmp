...
int ft_count_rotate_a(t_list **a, int val)
{
    int     count_rotate;
    t_list  *elem;
//    int     last_pos;
...
int ft_count_move(t_list **a, t_list **b, t_list *elem, t_moves *moves)
{
    ...
    if (moves->rrb < 0)
        moves->rrb = 0;
    count = ft_count_rotate_a(a, ELEM_VAL);
    moves->ra = count;
...
int ft_push_a(t_list **a, t_list **b, int med, t_moves *moves)
{
    t_list *elem;
    int count;
